...
#define MIN_ISR_START_LOOP_CYCLES                                                                                                                              \
  (ISR_START_X_STEPPER_CYCLES + ISR_START_Y_STEPPER_CYCLES + ISR_START_Z_STEPPER_CYCLES + ISR_START_E_STEPPER_CYCLES + ISR_START_MIXING_STEPPER_CYCLES)
...
#define MIN_PULSE_TICKS                                                                                                                                        \
  (((PULSE_TIMER_TICKS_PER_US) * (unsigned long)(MINIMUM_STEPPER_PULSE)) + ((MIN_ISR_START_LOOP_CYCLES) / (unsigned long)(PULSE_TIMER_PRESCALE)))
...
static FORCE_INLINE uint16_t MultiU16X8toH16(uint8_t charIn1, uint16_t intIn2)
{
  ...
  __asm__ __volatile__(A("clr %[tmp]") A("mul %[charIn1], %B[intIn2]") A("movw %A[intRes], r0") A("mul %[charIn1], %A[intIn2]") A("add %A[intRes], r1")
                           A("adc %B[intRes], %[tmp]") A("lsr r0") A("adc %A[intRes], %[tmp]") A("adc %B[intRes], %[tmp]") A("clr r1")
...
public:
  ...
  #endif
  static bool finishmov_flag, Zaxis_move;
...
// Mixing extruder mix delta_errors for bresenham tracing
  ...
  static uint32_t advance_dividend_m[MIXING_STEPPERS], advance_divisor_m;
#define MIXING_STEPPERS_LOOP(VAR) for (uint8_t VAR = 0; VAR < MIXING_STEPPERS; VAR++)
#else
  ...
  static uint16_t LA_current_adv_steps, LA_final_adv_steps,
      LA_max_adv_steps; // Copy from current executed block. Needed because current_block is set to NULL "too early".
  ...
  FORCE_INLINE static uint32_t calc_timer_interval(uint32_t step_rate, uint8_t scale, uint8_t *loops)
  {
    ...
    *loops = multistep;
    ...
    { // higher step rate
      ...
      const uint16_t table_address = (uint16_t)&speed_lookuptable_fast[(uint8_t)(step_rate >> 8)][0], gain = (uint16_t)pgm_read_word_near(table_address + 2);
    ...
    { // lower step rates
      ...
      timer = (uint16_t)pgm_read_word_near(table_address) - (((uint16_t)pgm_read_word_near(table_address + 2) * (uint8_t)(step_rate & 0x0007)) >> 3);
    }
