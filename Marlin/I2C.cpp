...
void I2C::scan()
{
  ...
  SERIAL_ECHOLNPGM("Scanning for devices...please wait");
  SERIAL_ECHOLNPGM(" ");
  for(uint8_t s = 0; s <= 0x7F; s++)
  {
    ...
    if(returnStatus)
    {
      if(returnStatus == 1)
      {
        SERIAL_ECHOLNPGM("There is a problem with the bus, could not complete scan");
    ...
    {
      SERIAL_ECHOLNPAIR("Found device at address - ", s);
      totalDevicesFound++;
    }
    stop();
  }
  if(!totalDevicesFound){ SERIAL_ECHOLNPGM("No devices found"); }
  timeOutDelay = tempTime;
}
